...
RBAPI(void) pwm_EnableMultiPin(unsigned long channels) {
    pwmdx_EnableMultiPin(channels & ~(0xffffffffL<<PWM_numChannels));
}
RBAPI(void) pwm_DisableMultiPin(unsigned long channels) {
    pwmdx_DisableMultiPin(channels & ~(0xffffffffL<<PWM_numChannels));
}
...
RBAPI(void) pwm_LockMulti(unsigned long channels) {
    pwmdx_LockMulti(channels & ~(0xffffffffL<<PWM_numChannels));
}
RBAPI(void) pwm_UnlockMulti(unsigned long channels) {
    pwmdx_UnlockMulti(channels & ~(0xffffffffL<<PWM_numChannels));
}
...
RBAPI(void) pwm_EnableMultiINT(unsigned long channels) {
    pwmdx_EnableMultiINT(channels & ~(0xffffffffL<<PWM_numChannels));
}
RBAPI(void) pwm_DisableMultiINT(unsigned long channels) {
    pwmdx_DisableMultiINT(channels & ~(0xffffffffL<<PWM_numChannels));
}

RBAPI(void) pwm_ClearMultiFLAG(unsigned long channels) {
    pwmdx_ClearMultiFLAG(channels & ~(0xffffffffL<<PWM_numChannels));
}
RBAPI(unsigned long) pwm_ReadMultiFLAG(unsigned long channels) {
    return pwmdx_ReadMultiFLAG(channels & ~(0xffffffffL<<PWM_numChannels));
}
...
RBAPI(void) pwm_EnableMultiPWM(unsigned long channels) {
    pwmdx_EnableMultiPWM(channels & ~(0xffffffffL<<PWM_numChannels));
}
RBAPI(void) pwm_DisableMultiPWM(unsigned long channels) {
    pwmdx_DisableMultiPWM(channels & ~(0xffffffffL<<PWM_numChannels));
}
...
RBAPI(unsigned long) pwm_ReadCtrlREG(int channel) {
    if ((channel<0) || (channel>=PWM_numChannels)) return 0xffffffffL;
