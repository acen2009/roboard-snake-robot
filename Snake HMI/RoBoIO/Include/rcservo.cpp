 ...
 * 
 * Remarks:
 * 1. In fact, we must read SB reg 0x6a-6b to get the base
 *    address of GPIO direction REG, and read SB reg 0x60-61, ...,
 *    0x68-69 to get the base address of GPIO0, ..., GPIO4 data
 *    REGs (Here, I am lazy, and directly use Vortex86DX's
 *    default setting, which is also the default setting of
 *    RoBoard's BIOS:p)
 * 2. When a GPIO pin is switch from input into output, it will
 *    always output 1.
 ********************************************************************/
static unsigned GPIO_port[4]    = {0x78, 0x79, 0x7a, 0x7c};
...
typedef struct
{
	...
	unsigned long invalidDuty;         //(unit: us) PWM duty that the servo will ignore
 ...
 ****************************************************************************/
	unsigned long invalidDuty = 5000L;
	int i;
    ...
    //NOTE: base address should be selected carefully to avoid I/O-space conflicts!
	if (pwm_Initialize(0xffff, PWMCLOCK_50MHZ, PWMIRQ_DISABLE) == false) return false;
    usedchannels = usedchannels & ~(0xffffffffL << pwm_NumCh());
	...
	for (i=0; i<32; i++)
	{
		...
		if (RCSERVO_svParams[i].servoType == RCSERVO_SV_DISABLE)
            rcservo_SetServo(i, RCSERVO_SERVO_DEFAULT);

		if (RCSERVO_svParams[i].invalidDuty > invalidDuty)
            invalidDuty = RCSERVO_svParams[i].invalidDuty;
	}

	//experimental trick to avoid RC servos' shake actions (invalid for KONDO's KRS motors)
	delay_ms(2L*invalidDuty/1000L);            //wait to make an invalid pulse for those channels that are input or nonzero output
	pwm_DisableMultiPWM(RCSERVO_usedChannels);
	pwm_EnableMultiPin(RCSERVO_usedChannels);  //all used channels now output 0
	delay_ms(2L*invalidDuty/1000L);
	...
	for (i=0; i<32; i++)
	{
        ...
        if (RCSERVO_svParams[i].servoType == RCSERVO_SV_FEEDBACK)
			pwm_SetPulseCount(i, 1L);
		else
			pwm_SetPulseCount(i, 0L);
	}
	pwm_EnableMultiPWM(RCSERVO_usedChannels); //initialize all RC servos with POWEROFF state

	for (i=0; i<32; i++)
	{
		if ((RCSERVO_usedChannels & (1L<<i)) == 0L) continue;
        while (pwm_ReadPulseCount(i) != 0L);  //should check timeout here, but I'm lazy:p
    }
	pwm_ClearMultiFLAG(RCSERVO_usedChannels);
...
RBAPI(void) rcservo_Close(void) {
	...
	pwm_DisableMultiPin(RCSERVO_usedChannels);
	pwm_Close();
...
static unsigned long rcservo_ReadPosition_normal(int channel, int cmd) {
	...
	pwm_SetPulseCount(channel, 1L);
    while (pwm_ReadPulseCount(channel) != 0);
	...
	pwm_SetPulseCount(channel, capTimeOut);
    while ((io_inpb(port) & pin) == 0)
		if ((width = pwm_ReadPulseCount(channel)) == 0L)
		{   //timeout
			width = 0xffffffffL;
			goto CAPTURE_END;
		}
	...
	pwm_DisablePWM(channel);
	pwm_SetPulseCount(channel, capMaxWidth);
	...
	pwm_DisablePWM(channel); //remain the channel's pull-up state
	pwm_SetPulse(channel, servo->invalidDuty+2L, 1L);
	pwm_SetPulseCount(channel, 1L);
	pwm_EnablePWM(channel);
    while (pwm_ReadPulseCount(channel) != 0L);
...
static unsigned long rcservo_ReadPosition_fast(int channel, int cmd) {
	...
	pwm_DisablePWM(channel); //set the channel to output 0; may cause a false pulse if OS context-switches here
	pwm_EnablePin(channel);
	io_outpb(dirport, io_inpb(dirport) | pin);
	io_outpb(port, io_inpb(port) & (~pin));
	pwm_DisablePin(channel);
...
RBAPI(unsigned long) rcservo_ReadPosition(int channel, int cmd) {
	...
	if ((RCSERVO_usedChannels & (1L<<channel)) == 0L) return 0xffffffffL;
...
_RB_INLINE void stop_pwmmode_pulses(void) {
	...
	{
		if ((RCSERVO_usedChannels & (1L<<i)) == 0L) continue;
		if (pwm_ReadPulseCount(i) != 0L) pwm_SetPulseCount(i, 1L);
	}

	for (i=0; i<32; i++)
	{
		if ((RCSERVO_usedChannels & (1L<<i)) == 0L) continue;
		while (pwm_ReadPulseCount(i) != 0L);
	}
}
...
_RB_INLINE void stop_playmode_pulses(void) {
	...
	pwm_ClearMultiFLAG(RCSERVO_validChannels);
	for (i=0; i<32; i++)
	{
	...
	while (pwm_ReadMultiFLAG(RCSERVO_validChannels) != RCSERVO_validChannels);
}
...
static unsigned long RCSERVO_playInterval = 50L; //50ms (20fps)
		...
		if ((width[i] < RCSERVO_svParams[i].minDuty) || (width[i] > RCSERVO_svParams[i].maxDuty))
		{
            switch (width[i])
            {
                case RCSERVO_MIXWIDTH_POWEROFF:
                    pwm_SetPulse(i, RCSERVO_svParams[i].period, RCSERVO_svParams[i].cmdDuty[RCSERVO_CMD_POWEROFF]);
			        pwm_SetCountingMode(i, PWM_CONTINUE_MODE);
			        break;
                case RCSERVO_MIXWIDTH_CMD1:
                    pwm_SetPulse(i, RCSERVO_svParams[i].period, RCSERVO_svParams[i].cmdDuty[RCSERVO_CMD1]);
			        pwm_SetCountingMode(i, PWM_CONTINUE_MODE);
			        break;
                case RCSERVO_MIXWIDTH_CMD2:
                    pwm_SetPulse(i, RCSERVO_svParams[i].period, RCSERVO_svParams[i].cmdDuty[RCSERVO_CMD2]);
			        pwm_SetCountingMode(i, PWM_CONTINUE_MODE);
			        break;
                case RCSERVO_MIXWIDTH_CMD3:
                    pwm_SetPulse(i, RCSERVO_svParams[i].period, RCSERVO_svParams[i].cmdDuty[RCSERVO_CMD3]);
			        pwm_SetCountingMode(i, PWM_CONTINUE_MODE);
			        break;
                case RCSERVO_MIXWIDTH_CMD4:
                    pwm_SetPulse(i, RCSERVO_svParams[i].period, RCSERVO_svParams[i].cmdDuty[RCSERVO_CMD4]);
			        pwm_SetCountingMode(i, PWM_CONTINUE_MODE);
			        break;
                case RCSERVO_MIXWIDTH_CMD5:
                    pwm_SetPulse(i, RCSERVO_svParams[i].period, RCSERVO_svParams[i].cmdDuty[RCSERVO_CMD5]);
			        pwm_SetCountingMode(i, PWM_CONTINUE_MODE);
			        break;
                case RCSERVO_MIXWIDTH_CMD6:
                    pwm_SetPulse(i, RCSERVO_svParams[i].period, RCSERVO_svParams[i].cmdDuty[RCSERVO_CMD6]);
			        pwm_SetCountingMode(i, PWM_CONTINUE_MODE);
			        break;
                case RCSERVO_MIXWIDTH_CMD7:
                    pwm_SetPulse(i, RCSERVO_svParams[i].period, RCSERVO_svParams[i].cmdDuty[RCSERVO_CMD7]);
			        pwm_SetCountingMode(i, PWM_CONTINUE_MODE);
			        break;
                default:
...
RBAPI(void) rcservo_EnterPlayMode(void) {
	...
	rcservo_ReadPositions_PlayMode(RCSERVO_curaction);
	for (i=0; i<32; i++)
    {
...
RBAPI(int) rcservo_PlayAction(void) {
    ...
    if (nowtime >= RCSERVO_curaction_nexttime)
	{
		...
		{
			if ((RCSERVO_validChannels & (1L<<i)) == 0L) continue;
		    midaction[i] = interpolate(RCSERVO_lastaction[i], RCSERVO_curaction[i], tt, t);
		}
...
RBAPI(int) rcservo_PlayActionMix(long* mixwidth) {
    static unsigned long nexttime = 0xffffffffL;
    ...
    if (RCSERVO_playState == RCSERVO_PLAYING)
    {
        ...
        { 
            ...
            add_mixwidth(midaction, mixwidth);
            set_playmode_pulses(midaction);
    ...
    if (RCSERVO_playState == RCSERVO_PLAYEND)
    {
        ...
        if (nowtime >= nexttime)
        {
            for (i=0; i<32; i++) midaction[i] = RCSERVO_curaction[i];
            add_mixwidth(midaction, mixwidth);
            set_playmode_pulses(midaction);
            while (nexttime <= nowtime) nexttime = nexttime + RCSERVO_playInterval;
        }
    }
...
void rcservo_PauseAction(void) {
	...
	RCSERVO_playState = RCSERVO_PAUSED;
	if ((nowtime = timer_nowtime()) >= RCSERVO_curaction_endtime)
    {
        RCSERVO_curaction_starttime = 0xffffffffL;
        return;
    }
	...
	{
		if ((RCSERVO_validChannels & (1L<<i)) == 0L) continue;
	    RCSERVO_lastaction[i] = interpolate(RCSERVO_lastaction[i], RCSERVO_curaction[i], tt, t);
	}
	RCSERVO_curaction_starttime = RCSERVO_curaction_endtime - nowtime;
}

void rcservo_ReleaseAction(void) {
	unsigned long nowtime;

	if (RCSERVO_playState != RCSERVO_PAUSED) return;

	RCSERVO_playState = RCSERVO_PLAYING;
	if (RCSERVO_curaction_starttime == 0xffffffffL) return;

	nowtime = timer_nowtime();
	RCSERVO_curaction_endtime   = RCSERVO_curaction_starttime + nowtime;
	RCSERVO_curaction_starttime = nowtime;
}
*/
...
RBAPI(void) rcservo_StopAction(void) {
	...
	if ((nowtime = timer_nowtime()) >= RCSERVO_curaction_endtime) return;
	...
	{
		if ((RCSERVO_validChannels & (1L<<i)) == 0L) continue;
	    RCSERVO_curaction[i] = interpolate(RCSERVO_lastaction[i], RCSERVO_curaction[i], tt, t);
	}
...
RBAPI(bool) rcservo_SendPWMPulses(int channel, unsigned long period, unsigned long duty, unsigned long count) {
	...
	if ((RCSERVO_usedChannels & (1L<<channel)) == 0L) return true;

	pwm_SetPulse(channel, period, duty);
	pwm_SetPulseCount(channel, count);
...
RBAPI(bool) rcservo_IsPWMCompleted(int channel) {
	if ((RCSERVO_usedChannels & (1L<<channel)) == 0L) return true;
	if (pwm_ReadPulseCount(channel) == 0L) return true; else return false;
}
...
RBAPI(void) rcservo_Outp(int channel, int value) {
    ...
    if ((channel < 0) || (channel >= pwm_NumCh()) || ((RCSERVO_usedChannels & (1L<<channel)) != 0L)) return;
...
RBAPI(int) rcservo_Inp(int channel) {
    ...
    if ((channel < 0) || (channel >= pwm_NumCh()) || ((RCSERVO_usedChannels & (1L<<channel)) != 0L)) return 0;
...
RBAPI(void) rcservo_Outps(unsigned long channels, unsigned long value) {
    ...
    channels  = channels & ~(0xffffffffL << pwm_NumCh());
    ...
    io_outpdw(GPIO_port[0], (io_inpdw(GPIO_port[0]) & (~gpio012ch)) | (value & gpio012ch));

    //output to GPIO port 4
    io_outpb(GPIO_dirPort[3], io_inpb(GPIO_dirPort[3]) | gpio4ch);
    io_outpb(GPIO_port[3], (io_inpb(GPIO_port[3]) & (~gpio4ch)) | ((unsigned char)(value >> 24) & gpio4ch));
}
...
RBAPI(unsigned long) rcservo_Inps(unsigned long channels) {
    ...
    io_outpdw(GPIO_dirPort[0], io_inpdw(GPIO_dirPort[0]) & (~gpio012ch));
    value = io_inpdw(GPIO_port[0]) & gpio012ch;
    
    //input from GPIO port 4
    io_outpb(GPIO_dirPort[3], io_inpb(GPIO_dirPort[3]) & (~gpio4ch));
    value = value | ((unsigned long)(io_inpb(GPIO_port[3]) & gpio4ch) << 24);
...
RBAPI(bool) rcservo_SetServoType(int channel, unsigned type, unsigned fbmethod) {
	...
	RCSERVO_svParams[channel].servoType = type;
...
RBAPI(bool) rcservo_SetServo(int channel, unsigned servono) {
	...
	switch (servono)
	{
		case RCSERVO_SERVO_DEFAULT:
            ...
            rcservo_SetServoParams2(channel, 10000L, 5000L);         //invalid duty = 10ms, min low-phase width  = 5000us
