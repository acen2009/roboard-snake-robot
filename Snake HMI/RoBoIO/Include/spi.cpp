/*******************************************************************************
  Update List
   ...
   2010/05/18:
    ...
    - [by acen] Add spi_InitializeSW(), spi_CloseSW(), spi_Exchange()

    - [by acen] Move spi_DisableCS(), spi_EnableCS(), spi_Write(), spi_WriteFlush(), 
	            spi_FIFOFlush(), spi_Read(), spi_Read2(), spi_ReadStrict() from spidx.cpp
	            to here (the old spi_DisableCS(), ... in spidx.cpp are renamed)
